...
def plot_cv_folds(pipeline, X, y, model_name, save_dir):
    ...
    # 创建4行3列的子图布局（5折 × 2个图/折 + 1个汇总图 = 11个子图）
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        # 绘制训练集图（第2*fold个子图）
        ax_train = axes[2*fold]
        ax_train.scatter(y_train_fold, y_train_pred, alpha=0.7, s=40, color='blue')
        
        # 绘制训练集对角线
        min_train = min(y_train_fold.min(), y_train_pred.min())
        max_train = max(y_train_fold.max(), y_train_pred.max())
        ax_train.plot([min_train, max_train], [min_train, max_train], 'k--', linewidth=1, alpha=0.7)
        
        ax_train.set_xlabel('True Values')
        ax_train.set_ylabel('Predicted Values')
        ax_train.set_title(f'{model_name} - Fold {fold+1} (Train)\nR² = {train_r2:.3f}')
        ax_train.grid(True, alpha=0.3)
        ax_train.set_xlim(min_train, max_train)
        ax_train.set_ylim(min_train, max_train)
        
        # 绘制验证集图（第2*fold+1个子图）
        ax_val = axes[2*fold + 1]
        ax_val.scatter(y_val_fold, y_val_pred, alpha=0.7, s=40, color='red')
        
        # 绘制验证集对角线
        min_val = min(y_val_fold.min(), y_val_pred.min())
        max_val = max(y_val_fold.max(), y_val_pred.max())
        ax_val.plot([min_val, max_val], [min_val, max_val], 'k--', linewidth=1, alpha=0.7)
        
        ax_val.set_xlabel('True Values')
        ax_val.set_ylabel('Predicted Values')
        ax_val.set_title(f'{model_name} - Fold {fold+1} (Validation)\nR² = {val_r2:.3f}')
        ax_val.grid(True, alpha=0.3)
        ax_val.set_xlim(min_val, max_val)
...
if __name__ == "__main__":
    ...
    model_save_path = r"E:\AAAA工作-研一\3济阳凹陷\5超压预测\2-机器学习方法\数据\multi_Eaton_index\第三次训练-分开验证"
